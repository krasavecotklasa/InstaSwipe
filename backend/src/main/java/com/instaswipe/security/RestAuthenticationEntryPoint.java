...
/**
 * Returns 401 with an {@link ApiError} body when a request is not authenticated. Replaces Spring
 * Security's default {@code Http403ForbiddenEntryPoint} (which a stateless, form-login-disabled
 * setup otherwise falls back to), so "not logged in" is a clear 401 rather than an opaque 403.
 */
public class RestAuthenticationEntryPoint implements AuthenticationEntryPoint {
    ...
    public void commence(HttpServletRequest request, HttpServletResponse response,
        ...
        SecurityResponseWriter.write(response, objectMapperProvider.getObject(),
                HttpStatus.UNAUTHORIZED.value(), failure.message());
    }
}
